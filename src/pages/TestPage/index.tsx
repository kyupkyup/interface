...
export default function TestPage() {
  ...
  const { account } = useWeb3React() // web3 react

  const [base, setBase] = useState<boolean>(false)
  const [fee] = React.useState(0.0005) // TODO: ***Get from backend***

  //TODO: setBase & setQuote based on current chain & selected pair
  const [baseCurrency] = useState<string>('ETH')
  const [quoteCurrency] = useState<string>('USDC')
    ...
    return (
      <div style={{ height: '200px', overflow: 'scroll' }}>
        {ownerOrderList.map((_order, index) => {
          const { base4Quote, amtIn, cumulAmtIn, cumulAmtOut, ownerOrderPos } = _order
          return (
